...
its own `.venv` (`uv sync`) and its own `db.sqlite3` - SQLite needs no
server or config to isolate a worktree, it just needs its own file, which
it gets automatically. The orchestrator sets it up and tells you where it
is.

Write tests as `uv run python manage.py test chores_app` (Django's test
runner, not pytest - nothing in this project pulls in pytest). Read
`AGENTS.md` for the current architecture before touching models, and
`_docs/decisions.md` before assuming an open question is actually open.
